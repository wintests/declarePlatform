...
<title>用户信息管理</title>
	...
	<script type="text/javascript">
		...
		function removeUser() {
			...
			//判断是否有选择的行
			if (selectedRows.length == 0) {
				$.messager.alert("系统提示","<font size='2'>请至少选择一条要删除的数据！</font>", "info");
				return;
			}
			//定义选中 选中user_name数组
			var ids = [];
		...
		function enabledBatchs() {
			...
			//判断是否有选择的行
			if (selectedRows.length == 0) {
				$.messager.alert("系统提示","<font size='2'>请选择需要启用的用户！</font>", "info");
				return;
			}
			//定义选中 选中user_name数组
			var ids = [];
		...
		function disabledBatchs() {
			...
			//判断是否有选择的行
			if (selectedRows.length == 0) {
				$.messager.alert("系统提示","<font size='2'>请选择需要禁用的用户！</font>", "info");
				return;
			}
			//定义选中 选中user_name数组
			var ids = [];
		...
		function changeStatusBatchs(r, ids, signln_valid) {
			if (r) {
				...
				function(data) {
					if (data.state) {
						$.messager.alert("系统提示","<font size='2'>恭喜您，更改状态成功！</font>", "info");
						$("#dg").datagrid("unselectAll");
						$("#dg").datagrid("reload");
					} else {
						$.messager.alert("系统提示", "<font size='2'>" + data.message + "</font>", "error");
